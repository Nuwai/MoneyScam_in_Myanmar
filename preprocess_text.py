...
def remove_control_chars(text):
    """
    Removes ASCII control characters that Excel rejects (e.g., STX 0x02).

    Args:
        text (str): Input text.

    Returns:
        str: Text without disallowed control characters.
    """
...
def tokenize_burmese(text, token_method='Syllable'):
    """
    Tokenizes Burmese text using the specified tokenizer.

    Args:
        text (str): The input Burmese text.
        token_method (str): Tokenization method to use ('Syllable', 'word', or fallback Myanmar tokenizer).
    ...
    if token_method == 'Syllable':
        ...
        tokens = tokenizer.tokenize(text)
    elif token_method == 'mmdt':
        tokenizer = MyanmarTokenizer()
        tokens = tokenizer.word_tokenize(text)[0]
    else:
        raise ValueError("Invalid token_method. Choose 'Syllable', 'word', or 'mmdt'.")

    return tokens

def remove_burmese_stopwords(text, token_method='mmdt'):
    """
    ...
    tokens = tokenize_burmese(text, token_method)
...
def combine_text(text):
    ...
    burmese_text = remove_burmese_stopwords(burmese_text, token_method='mmdt')  
...
def preprocess_text(text):
    ...
    Steps:
    0. Removes disallowed control characters (e.g., STX 0x02) using `remove_control_chars()`.
    ...
    """
    # Step 0: Remove control characters that Excel rejects (e.g., STX 0x02)
...
def load_data_and_run(save=True):
    ...
    save_path=os.path.join(script_dir,"data/preprocess_mmdt_tokenizer.xlsx")
    # Load the dataset
